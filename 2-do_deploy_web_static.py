...
def do_deploy(archive_path):
    ...
    if run(f'mkdir -p {releases}/{archive_path[9:-4]}/').failed:
        return False
    ...
    if run(f'mv {web_static_files} {new_release_folder}').failed:
        return False

    # Delete the web_static folder (result of uncompressing)
    if run(f'rm -rf {releases}/{archive_path[9:-4]}/web_static').failed:
        return False

    # Delete the archive from /tmp
    if run(f'rm /tmp/{archive_path[9:]}').failed:
        return False

    # Delete the symbolic link /data/web_static/current
    if run(f'rm {current}').failed:
        return False

    # Create new symbolic link, current->
    if run(f'ln -s {releases}/{archive_path[9:-4]} {current}').failed:
        return False
